...
COPY --from=builder /fastapi-mvc-example /fastapi-mvc-example

ENV PYTHONDONTWRITEBYTECODE=1
ENV PYTHONUNBUFFERED=1
ENV PATH="/fastapi-mvc-example/.venv/bin:$PATH"

EXPOSE 8000/tcp

STOPSIGNAL SIGINT

CMD ["/fastapi-mvc-example/.venv/bin/fastapi-mvc-example", "serve", "--host", "0.0.0.0"]
